...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="fr" lang="fr">
	...
	<head>
		...
		<title>Ovnie - Not Authorized | Projet 7.20.12.0.13</title>
	</head>
		...
		if(isset($_POST['lancerlaverification'])){
			...
			if (isset($le_code) AND !empty($le_code)){
				if ($le_code == "ange" OR $le_code == "anges"){
					...
					$erreur_passe = "Le mot de passe inséré est incorrect, vous n'êtes pas autorisé à accéder à la zone interdite !";
				}
			}
			else{
				$erreur_passe = "Le mot de passe inséré est vide, vous n'êtes pas autorisé à accéder à la zone interdite !";
			}
		}?>
		<div class="secret">
			<a href="https://ovnie.com"><img class="bloc" src="images/secretHeader.png" alt="Image" title="Ovnie - Not Authorized" /></a>
		</div>
		<div class="secret">
			...
			<form method="post">
				<label for="ticket">Entrer le mot de passe pour accéder à la zone interdite :</label>
				<p><input type="text" name="codesecret" id="codesecret" /></p>
				<p><input type="submit" value="Envoyer" name='lancerlaverification' /></p>
			</form>
			<img class="bloc" src="images/notauthorized2.png" alt="Image" title="Ovnie - Not Authorized" />
		</div>
	</body>
